...
void findAllPaths(int x, int y, char map[N][N], char sol[N][N], string move) {
    // Check if we have reached the endpoint 'E'
    if (map[x][y] == 'E') {
        cout << "Solution: " << endl;
        sol[x][y] = '.';
        noSolution = true;
        ...
        sol[x][y] = 'E';  // Reset to avoid affecting other paths
        return;
    }

    // Check if it's a valid cell to visit
    if (x >= 0 && y >= 0 && x < N && y < N && map[x][y] != 'X' && sol[x][y] == '0') {
        sol[x][y] = '.';  // Mark current cell as visited
        ...
        sol[x][y] = '0';  // Unmark for backtracking
    }
}
